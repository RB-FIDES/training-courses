...
```
Функція `source()` запускає скрипт `activate_ukrainian.R`, який налаштовує український інтерфейс для курсів swirl. Проту ви можете запустити його вручну, або ж скористатися функцією `activate()`.
