...
```
Функція `source()` запускає скрипт `activate_ukrainian.R`, який налаштовує український інтерфейс для курсів swirl. Проте ви можете запустити його вручну, або ж скористатися функцією `activate()`.
...
2) English
```
Вибираємо `1` для української мови. Після цього ви зможете проходити курси українською мовою.

### Крок 3.1: Вибір курсу
```r
Доступні опції?
1) Продовжити
2) Встановити
...
- Після того як ти закінчив налаштування за допомогою `activate()`, ти можеш запускати swirl як звичайно, просто викликавши `swirl()` (функція `activate()` зникає - керуєш через команди, або перезапускає активацію)
